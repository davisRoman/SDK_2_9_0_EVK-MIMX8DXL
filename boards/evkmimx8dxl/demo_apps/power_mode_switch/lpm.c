...
typedef struct _lpm_nvic_context
{
    ...
    uint32_t ISER[8];
    uint8_t IP[240]; /* 8DXL CM4 max IRQn is 50 */
    uint8_t SHP[12];
} lpm_nvic_context_t;
...
static void LPM_GptInit(void)
{
    ...
    err = sc_pm_set_resource_power_mode((sc_ipc_t)IPC_MU, SYSTICK_RSRC, SC_PM_PW_MODE_ON);
    if (SC_ERR_NONE != err)
    {
        assert(false);
    }
...
{
    uint64_t counter, timeoutTicks;
    uint32_t flag, expired = 0;
    ...
    err = sc_pm_set_resource_power_mode((sc_ipc_t)IPC_MU, SYSTICK_RSRC, SC_PM_PW_MODE_OFF);
    if (SC_ERR_NONE != err)
    {
        assert(false);
    }
...
bool LPM_WaitForInterrupt(uint32_t timeoutMilliSec)
{
     ...
     * up.*/
    err = sc_pm_set_resource_power_mode((sc_ipc_t)IPC_MU, IRQSTEER_RSRC, SC_PM_PW_MODE_ON);
    if (SC_ERR_NONE != err)
    {
        assert(false);
    }
