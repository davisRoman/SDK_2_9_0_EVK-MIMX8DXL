...
static status_t BOARD_GetIOExpanderI2CAndAddress(board_io_expander_pin_t pin, LPI2C_Type **lpi2c, uint8_t *addr)
{
    status_t status = kStatus_Fail;

    static LPI2C_Type *const i2cs[] = {NULL, ADMA__LPI2C2, ADMA__LPI2C2, ADMA__LPI2C3, SCU__LPI2C};
    static const uint8_t addrs[]    = {0U, 0x20U, 0x21U, 0x20U, 0x20U};

    uint8_t ioexp = BOARD_IOEXP_GET_EXPANDER(pin);

    if (ioexp < ARRAY_SIZE(i2cs))
    {
        *lpi2c = i2cs[ioexp];
        *addr  = addrs[ioexp];
...
{
    uint8_t regVal;
    uint8_t command  = PCA6416A_CONFIG_PORT0;
    uint8_t pinIndex = BOARD_IOEXP_GET_PININDEX(pin);
    uint8_t addr;
    LPI2C_Type *lpi2c;
    status_t status;

    status = BOARD_GetIOExpanderI2CAndAddress(pin, &lpi2c, &addr);
    ...
    /* Check which command register to touch. */
    if (pinIndex >= 8)
    {
        pinIndex &= 0x07U;
        command += 1U;
    ...
    status = BOARD_LPI2C_Receive(lpi2c, addr, command, 1, &regVal, 1);
    if (kStatus_Success != status)
    {
        return status;
    }

    if (kIOEXP_PinInput == dir)
    {
        regVal |= (1U << pinIndex);
    }
    else
