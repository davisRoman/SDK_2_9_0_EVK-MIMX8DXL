 ...
 * Definitions
 ******************************************************************************/
#define EXAMPLE_CAN             ADMA__CAN2
#define EXAMPLE_CAN_DMA         ADMA__EDMA3
#define EXAMPLE_CAN_DMA_CHANNEL (14)
/*
 * When CLK_SRC=1, the protocol engine works at fixed frequency of 160M.
 * If other frequency wanted, please use CLK_SRC=0 and set the working frequency for CAN_2.
 */
...
int main(void)
{
    ...
    /* Power on the FlexCAN0，CAN1/CAN2 share the clock from CAN0, and their power domain is the child domain of CAN0's.
     */
    ...
    /* Power on EDMA . */
    if (sc_pm_set_resource_power_mode(ipc, SC_R_DMA_3_CH14, SC_PM_PW_MODE_ON) != SC_ERR_NONE)
    {
    ...
    IRQSTEER_EnableInterrupt(IRQSTEER, ADMA_FLEXCAN2_DMA_INT_IRQn);
