...
#define PHY_MDIO_MMD_PCS 3
#define PHY_MDIO_MMD_AN  7

/*! @brief MDIO MMD Physical Coding layer device registers .*/
#define PHY_MDIO_PCS_EEE_CAP 0x14 /* EEE capability */

/*! @brief MDIO MMD AutoNegotiation device registers .*/
#define PHY_MDIO_AN_EEE_ADV 0x3c /* EEE advertisement */

/*! @brief MDIO MMD EEE mask flags. (common for adv and cap) */
#define PHY_MDIO_EEE_100TX 0x2
#define PHY_MDIO_EEE_1000T 0x4
...
status_t PHY_AR8031_Init(phy_handle_t *handle, const phy_config_t *config)
{
    ...
    if (result == kStatus_Success)
    {
        ...
        result = MDIO_Write(handle->mdioHandle, handle->phyAddr, PHY_DEBUGPORT_DATA_REG, (regValue | 0x0100));
        if (result != kStatus_Success)
        {
            return result;
        }
        ...
        result = MDIO_Write(handle->mdioHandle, handle->phyAddr, PHY_DEBUGPORT_DATA_REG, (regValue | 0x8000));
        if (result != kStatus_Success)
        {
            return result;
        }
...
status_t PHY_AR8031_GetLinkSpeedDuplex(phy_handle_t *handle, phy_speed_t *speed, phy_duplex_t *duplex)
{
    ...
    if (result == kStatus_Success)
    {
        ...
        if (duplex != NULL)
        {
            if ((regValue & PHY_SSTATUS_LINKDUPLEX_MASK))
            {
                                         ...
                                         phy_mmd_access_mode_t mode)
{
    ...
    result = MDIO_Write(handle->mdioHandle, handle->phyAddr, PHY_MMD_ACCESS_CONTROL_REG, mode | device);
    return result;
}
